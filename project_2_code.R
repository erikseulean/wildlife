...
model{
  ...
  log.n1 ~ dnorm(0.92333, 0.01)
  log.N[1] <- log.n1 
  b0 ~ dnorm(0, 0.00001) #Prior for the Intercept
  b1 ~ dnorm(0, 0.00001) #Prior for the slope
  sig.n ~ dunif(0, 1)  #Variance for the state process 
  sig2.n <- pow(sig.n, 2) #Standard Devation for the state process
  tau.n <- pow(sig.n, -2) #Precision for the state process
  sig.y ~ dunif(0, 1) #Variance for the observations
  sig2.y <- pow(sig.y, 2)
  tau.y <- pow(sig.y, -2)
...
wildebeest_dta <- list(rain = c(wildebeest$rain, rep(NA,projection)), y = c(log(wildebeest$Nhat), rep(NA,projection)) , nyrs = nrow(wildebeest)+projection)
